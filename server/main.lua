...
RegisterNetEvent('tr-mining:BuyPickaxe', function()
    ...
    local pickaxe = Player.Functions.GetItemByName('weapon_stone_hatchet')
    if not pickaxe then
        Player.Functions.AddItem('weapon_stone_hatchet', 1)
        TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['weapon_stone_hatchet'], "add")
...
QBCore.Functions.CreateCallback('tr-mining:pickaxe', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
        if Player.Functions.GetItemByName("weapon_stone_hatchet") ~= nil then
            cb(true)
        else
            cb(false)
        end
...
RegisterNetEvent('tr-mining:receivedReward')
    AddEventHandler('tr-mining:receivedReward', function()
    ...
    if not "miner" then
    TriggerClientEvent('QBCore:Notify', source, 'Não és mineiro', 'error')
    else
